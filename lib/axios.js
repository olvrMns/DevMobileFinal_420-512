...
export const api = axios.create({
    baseURL:"http://192.168.2.36:5555"
})
...
export const getGames = async (paramsObj, page_size) => {
    try {
        const games = await api.post("/games", {params: paramsObj, page_size: page_size});
        return games.data;
    } catch (ex) {
...
export const handleFavorite = async (userId, gameId, interaction) => {
    let res;
    try {
        switch (interaction) {
            case "add":
                res = await api.post("/games/add-favorite", {userId: userId, gameId: gameId});
                break;
            case "remove":
                res = await api.post("/games/remove-favorite", {userId: userId, gameId: gameId});
                break;
            case "check":
                res = await api.post("/games/check-favorite", {userId: userId, gameId: gameId});
                break;
        }
        return res.data;
    } catch (ex) {
...
export async function updateFriendDescription(userId,friendId,description){
    try{
        ...
        const updateFriend=await api.put(`/users/${userId}/friends`,updateDescriptionData);
        return updateFriend.data
    }catch(error){
...
export async function deleteFriend(userId,friendId){
    try{
        ...
        const deleteFriend=await api.delete(`/users/${userId}/friends`,{data:deleteData});
